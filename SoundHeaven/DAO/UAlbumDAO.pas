...
begin
  ...
  try
    try
      ...
      on E: Exception do
      begin
        raise Exception.Create('Erro ao atualizar álbum: ' + E.Message);
      end;
    end;
  finally
    StoredProc.Free;
...
var
  ...
  Bitmap: TBitmap;
begin
  Result := nil; // Inicializa o resultado como nil por precaução
  StoredProc := TADOStoredProc.Create(nil);
  try
    with StoredProc do
    begin
      ...
      begin
        ...
        Result := TAlbum.Create(
          ...
          nil // Capa será carregada em seguida
        );

        // Carregar a capa se não for null
    ...
    begin
      Result := nil;
      raise Exception.Create('Erro ao localizar o álbum: ' + E.Message);
    end;
  end;
  StoredProc.Free;
end;
